...
interface CommandSetTextOnLoadedRecordInterface {
    details: string
    ...
    notYetCommit?: boolean // if true, then command will not commit the line, but will set the value and wait for sourcing
}
...
export class CommandSetTextOnLoadedRecord implements Command {
    details: string;
    targetObjectType: Type | string;
    targetObjectId: string;
    group: string;
    type: string = `CommandSetTextOnLoadedRecord`;
    previousValue: string;
    valueToSet: string;
    field: string;
    sublistId: string | null;
    ...
    [key: string]: unknown;
    constructor(args: CommandSetTextOnLoadedRecordInterface) {
    ...
    conditionsToStop(context: CommandContext): CommandContext {
        if (!context || !context.loadedRecords || !context.loadedRecords[this.targetObjectId]) {
            ...
            try {
                ...
                context.failed = true;
                context.result = `For CommandSetTextOnLoadedRecord context must contain loaded record with the same id as targetObjectId: "${this.targetObjectId}" (error: ${JSON.stringify(e)})`;
                return context;
            }
        }
        return context;
        ...
        try {
            ...
            if (this.sublistId) { // means we are setting sublist value, not body value
                ...
                if (loadedRecord.isDynamic) { // for dynamic record we need to select line before setting sublist value and then commit it
                    ...
                    loadedRecord.setCurrentSublistText({
                        sublistId: this.sublistId,
                        fieldId: this.field,
                        text: this.valueToSet,
                        fireSlavingSync: this.needToWaitForSourcing || needToWaitForFieldSourcing({fieldId: this.field, sublistId: this.sublistId})
                    });
                    if (!this.notYetCommit) { // some fields cannot be set without other fields being set, so we must not committing the line
    ...
    static fromString(s: string): Command {
        const obj = JSON.parse(s) as CommandSetTextOnLoadedRecord;
        return new CommandSetTextOnLoadedRecord({
            details: obj.details,
